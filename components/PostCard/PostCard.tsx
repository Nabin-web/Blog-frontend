...
const PostCard = (post: any) => {
  ...
  const [allReview, setAllReview] = React.useState<allReviews[]>([])
  ...
  const fetchReview = async () => {
    try {
      const data = await axios.get(
        'http://localhost:5000/review/show',
        auth.config
      )
  ...
  useEffect(() => {
    ...
    setReview({
      ...review,
      teamid: localStorage.getItem('userId')!!,
      teamname: localStorage.getItem('userName')!!,
    })
  ...
  return (
    <div className="mb-8 rounded-lg bg-white p-0 pb-12 shadow-lg lg:p-8">
      <div className="relative mb-6 overflow-hidden pb-80 shadow-md">
        <img
          src="http://lifestylelinks.net/wp-content/uploads/2021/09/opener-home-workspace.jpg"
          alt="react-img"
      ...
      <div className="text-center">
        <Link href="/">
          <span className="font-medium, inline-block transform rounded-full bg-pink-600 px-8 py-3 text-lg text-white transition duration-500 hover:-translate-y-1">
            COntinue Reading
          </span>
        </Link>
      </div>
      ...
      {allReview.map((review) => {
        return (
          review.postId === post.post._id && (
            <>
              <div className="mb-8 mt-4 block w-full items-center text-center lg:flex ">
              ...
              <p className="text-md mb-8  px-4 font-normal text-gray-700 ">
                {review.comment}
              </p>
            </>
          )
      ...
      <input
        ...
        className="border-grey-light mb-4 mt-4 block w-full rounded border p-3"
        name="comment"
        placeholder="Comment"
      />
      ...
      <button
        ...
        className=" font-medium, inline-block transform rounded-full bg-green-600 px-8 py-3 text-lg text-white"
      >
        Comment
      </button>
    </div>
